...
function VsRow({ oppName, oppColour, oppFlag, dim }: { oppName: string; oppColour: string; oppFlag: string; dim?: boolean }) {
  return (
    <div className={clsx("flex items-center justify-center gap-5 sm:gap-10 transition-opacity", dim && "opacity-40")}>
...
export default function KnockoutScreen() {
  const {
    simResults, simMeta, knockoutIdx, knockoutPhase,
  ...
  if (!simMeta || !match) return null;

  const oppFlag = FLAGS[match.oppCode] ?? "🏏";
  const isLast = globalIdx === simResults.length - 1;

  return (
    <section className="flex-1 min-h-0 max-w-3xl w-full mx-auto px-5 py-6 flex flex-col items-center justify-center">
      <div className="relative w-full bg-panel border border-line rounded-3xl shadow-2xl overflow-hidden">
        ...
        <div className="relative text-center pt-6 pb-1">
          <span className="inline-block text-[12px] font-black tracking-[3px] uppercase text-accent bg-accent/10 border border-accent/30 rounded-full px-4 py-1.5">
            {match.stage}
          </span>
        </div>
            ...
            >
              ...
              <div className="text-center mt-6">
                <Button size="lg" onClick={continueKnockout}>
                  {isLast ? "See Final Result →" : `Next: ${simResults[globalIdx + 1]?.stage} →`}
                </Button>
              </div>
            </motion.div>
          )}
